...
static int tcp_prepare_devmem_data(struct msghdr *msg, int devmem_fd,
				   int devmem_offset, struct file **devmem_file,
				   struct iov_iter *devmem_tx_iter, size_t size)
{
	...
	*devmem_file = fget_raw(devmem_fd);
	...
	if (devmem_offset >= priv->dmabuf->size) {
		err = -ESPIPE;
		goto err_fput;
	}

	if (priv->dmabuf->size - devmem_offset < size) {
		err = -ENOSPC;
		goto err_fput;
	}
			      ...
			      int offset, struct msghdr *msg, int len)
{
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
		...
		if (copy > 0) {
			...
			get_page(page);
			devmemvec.frag_token = user_token;
